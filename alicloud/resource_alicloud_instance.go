...
func resourceAliyunInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("creation_time", instance.CreationTime.String())
	...
	// check parameter stop
	if !(instance.Status == ecs.Running || instance.Status == ecs.Stopped) {
		err := resource.Retry(8*time.Minute, func() *resource.RetryError {
			instanceTemp, _ := client.QueryInstancesById(d.Id())

			if instanceTemp.Status == ecs.Running || instanceTemp.Status == ecs.Stopped {
				instance.Status = instanceTemp.Status
				d.Set("status", instance.Status)

				return nil
			} else {
				return resource.RetryableError(fmt.Errorf("Get instance status timeout."))
			}
		})
	...
	if stop != "0" && instance.Status == ecs.Running {
		// try to stop the instance
		if err := conn.StopInstance(d.Id(), false); err != nil {
			return fmt.Errorf("StopInstance got error: %#v", err)
		}
		if err := conn.WaitForInstanceAsyn(d.Id(), ecs.Stopped, 300); err != nil {
			return fmt.Errorf("WaitForInstance %s got error: %#v", ecs.Stopped, err)
		}
		instance.Status = ecs.Stopped
		d.Set("status", instance.Status)
	}
	if stop == "0" && instance.Status == ecs.Stopped {
	...
	mapping := map[string]interface{}{
		...
		"creation_time":        instance.CreationTime.String(),
		"resource_type":        "alicloud_instance",
		"region_id":            instance.RegionId,
		"zone_id":              instance.ZoneId,
		"instance_type":        instance.InstanceType,
		"host_name":            instance.HostName,
		"system_disk_id":       d.Get("system_disk_id").(string),
		...
		"disk_device_mappings": instanceDisksMappings(d, instance.InstanceId, meta),
		"tags":                 tagsToMap(instance.Tags.Tag),
		/*
			"image_id":                   instance.ImageId,
			"security_groups":            instance.SecurityGroupIds.SecurityGroupId,
			"eip":                        instance.EipAddress.IpAddress,
			"key_name":                   instance.KeyPairName,
	...
	if len(instance.InnerIpAddress.IpAddress) > 0 {
		mapping["private_ip"] = instance.InnerIpAddress.IpAddress[0]
	} else {
		mapping["private_ip"] = instance.VpcAttributes.PrivateIpAddress.IpAddress[0]
	}
	/*
		if len(instance.PublicIpAddress.IpAddress) > 0 {
			mapping["public_ip"] = instance.PublicIpAddress.IpAddress[0]
		} else {
			mapping["public_ip"] = instance.VpcAttributes.NatIpAddress
		}
	*/
	...
	if err := d.Set("alicloud_instance", s); err != nil {
		return fmt.Errorf("Setting alicloud_instance got an error: %#v.", err)
	}
