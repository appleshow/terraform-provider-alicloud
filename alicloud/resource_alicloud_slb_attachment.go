...
func resourceAliyunSlbAttachmentRead(d *schema.ResourceData, meta interface{}) error {
	...
	mapping := map[string]interface{}{
		...
		"weight":          weight,
		"backend_servers": strings.Join(instanceIds, ","),
	}
	s = append(s, mapping)

	if err := d.Set("alicloud_slb_attachment", s); err != nil {
		return fmt.Errorf("Setting alicloud_slb_attachment got an error: %#v.", err)
	}
