...
func validateInstanceDescription(v interface{}, k string) (ws []string, errors []error) {
	value := v.(string)
	if value != "" && (len(value) < 2 || len(value) > 256) {
		errors = append(errors, fmt.Errorf("%q must be 2 ~ 256 characters", k))
...
func validateCommonResourceName(v interface{}, k string) (ws []string, errors []error) {
	value := v.(string)
