...
func dataSourceAlicloudImagesRead(d *schema.ResourceData, meta interface{}) error {
	...
	if idOK == false && nameRegexOk == false && ownersOk == false && mostRecentOk == false {
		return fmt.Errorf("One of id, name_regex, owners or most_recent must be assigned")
	}
...
func imagesDescriptionAttributes(d *schema.ResourceData, images []ecs.ImageType, meta interface{}) error {
	...
	if err := d.Set("alicloud_images", s); err != nil {
		return err
	}
