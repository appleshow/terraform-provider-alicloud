...
func resourceAliyunSecurityGroupRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	mapping := map[string]interface{}{
		...
		"rule_type":                  rule.Direction,
		"ip_protocol":                strings.ToLower(string(rule.IpProtocol)),
		"nic_type":                   rule.NicType,
		"policy":                     strings.ToLower(string(rule.Policy)),
		"port_range":                 rule.PortRange,
		"priority":                   rule.Priority,
		"security_group_id":          sgId,
		"cidr_ip":                    d.Get("cidr_ip").(string),
		"source_security_group_id":   d.Get("source_security_group_id").(string),
		"source_group_owner_account": d.Get("source_group_owner_account").(string),
	}
	s = append(s, mapping)

	if err := d.Set("alicloud_security_group_rule", s); err != nil {
		return fmt.Errorf("Setting alicloud_security_group_rule got an error: %#v.", err)
	}
