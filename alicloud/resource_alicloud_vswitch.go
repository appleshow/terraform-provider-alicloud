...
func resourceAliyunSwitchRead(d *schema.ResourceData, meta interface{}) error {
	...
	mapping := map[string]interface{}{
		"id":            d.Id(),
		"vpc_id":        vswitch.VpcId,
		"zone_id":       vswitch.ZoneId,
		"name":          vswitch.VSwitchName,
		"cidr_block":    vswitch.CidrBlock,
		"description":   vswitch.Description,
		"is_default":    vswitch.IsDefault,
		"creation_time": time.Now().Format("2006-01-02 15:04:05"),
		"status":        "Available",
		"resource_type": "alicloud_vswitch",
	}
	s = append(s, mapping)

	if err := d.Set("alicloud_vswitch", s); err != nil {
		return fmt.Errorf("Setting alicloud_vswitch got an error: %#v.", err)
	}
