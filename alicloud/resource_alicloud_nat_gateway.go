...
func resourceAliyunNatGatewayRead(d *schema.ResourceData, meta interface{}) error {
	...
	mapping := map[string]interface{}{
		...
		"specification":         natGateway.Spec,
		"description":           natGateway.Description,
		"bandwidth_package_ids": strings.Join(natGateway.BandwidthPackageIds.BandwidthPackageId, ","),
		"snat_table_ids":        strings.Join(natGateway.SnatTableIds.SnatTableId, ","),
		"forward_table_ids":     strings.Join(natGateway.ForwardTableIds.ForwardTableId, ","),
	}
	s = append(s, mapping)

	if err := d.Set("alicloud_nat_gateway", s); err != nil {
		return fmt.Errorf("Setting alicloud_nat_gateway got an error: %#v.", err)
	}
